...
public class NativeOnboardFullscreen1 {

    private static NativeOnboardFullscreen1 nativeOnboardFullscreen1;
    ...
    public static NativeOnboardFullscreen1 getInstance() {
        if (nativeOnboardFullscreen1 == null) {
            nativeOnboardFullscreen1 = new NativeOnboardFullscreen1();
        }
        return nativeOnboardFullscreen1;
    }
    ...
    private void loadAdsHigh(final Activity activity) {
        try {
            ...
            if (!PurchaseUtils.isNoAds(activity) && FirebaseQuery.getEnableAds() && FirebaseQuery.getEnableNative()) {
                if (FirebaseQuery.getEnableNativeFullScr1High()) {
                    isLoadingHigh = true;
                    AdLoader.Builder builder = new AdLoader.Builder(activity, FirebaseQuery.getIdNativeFullOnboard1High()); // Store thay lại id
    ...
    private void loadAdsNormal(final Activity activity) {
        try {
            ...
            if (!PurchaseUtils.isNoAds(activity) && FirebaseQuery.getEnableAds() && FirebaseQuery.getEnableNative() && FirebaseQuery.getEnableNativeFullScr1()) {
                isLoadingNormal = true;
                AdLoader.Builder builder = new AdLoader.Builder(activity, FirebaseQuery.getIdNativeFullOnboard1()); // Store thay lại id
                builder.forNativeAd(new NativeAd.OnNativeAdLoadedListener() {
                    @Override
                    public void onNativeAdLoaded(@NonNull NativeAd nativeAd) {
    ...
    public void loadAdsAll(Activity activity) {
        ...
        } else if (FirebaseQuery.getEnableNativeFullScr1()) {
            loadAdsNormal(activity);
        }
    }
