            ...
            if (!PurchaseUtils.isNoAds(activity) && FirebaseQuery.getEnableAds() && FirebaseQuery.getEnableNative() && FirebaseQuery.getEnableNativeLanguageDup()) {
                ...
                AdLoader adLoader = builder.withAdListener(new AdListener() {
                    ...
                    public void onAdLoaded() {
                        Log.e("TamBT", "onAdLoaded: native language dup normal");
                        super.onAdLoaded();
                        isLoadingNormal = false;
                    ...
                    public void onAdFailedToLoad(@NonNull LoadAdError loadAdError) {
                        Log.e("TamBT", "onAdFailedToLoad: native language dup normal " + loadAdError.getMessage());
                        isLoadingNormal = false;
                        if (callbackNative != null) {
    ...
    private void loadAdsHigh(final Activity activity) {
        try {
            ...
            if (!PurchaseUtils.isNoAds(activity) && FirebaseQuery.getEnableAds() && FirebaseQuery.getEnableNative()) {
                if (FirebaseQuery.getEnableNativeLanguageDupHigh()) {
                    isLoadingHigh = true;
                    AdLoader.Builder builder = new AdLoader.Builder(activity, FirebaseQuery.getIdNativeLanguageDupHigh());
                    builder.forNativeAd(new NativeAd.OnNativeAdLoadedListener() {
                        @Override
                        public void onNativeAdLoaded(@NonNull NativeAd nativeAd) {
                            isLoadingHigh = false;
                    ...
                    AdLoader adLoader = builder.withAdListener(new AdListener() {
                        ...
                        @Override
                        public void onAdLoaded() {
                            Log.e("TamBT", "onAdLoaded: native language dup high");
                            super.onAdLoaded();
                            isLoadingHigh = false;
                            if (callbackNative != null) {
                                callbackNative.onLoaded();
                        ...
                        @Override
                        public void onAdFailedToLoad(@NonNull LoadAdError loadAdError) {
                            Log.e("TamBT", "onAdFailedToLoad: native language dup high " + loadAdError.getMessage());
                            isLoadingHigh = false;
